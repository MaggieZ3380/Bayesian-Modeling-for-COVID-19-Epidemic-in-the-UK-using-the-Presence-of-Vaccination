...
#data from stan
data_seir_v_2_peaks <- list(n_days = n_days, n_cases=length(cases),
                            t0 = t0, ts = t, x_r= c(n_days,v), cases = cases, N = N)
```

```{r}
seir_v_2_peaks_model <- stan_model('2_peaks.stan')
fit_seir_V_2_peaks <- sampling(seir_v_2_peaks_model, 
                    data = data_seir_v_2_peaks, 
                    iter =1000,
                    chains = 4)
```
...
print(fit_seir_V_2_peaks, pars = c('lambda1', 'lambda2','sigma', 'mu', 'p_reported', 'R0_1', 'R0_2'))
```


```{r}
#posterior density for separate chain
stan_dens(fit_seir_V_2_peaks, pars = c('lambda1', 'lambda2','sigma', 'mu', 'p_reported', 'R0_1', 'R0_2'), separate_chains = TRUE)
```



```{r}
SMR_pred_2 <- cbind(as.data.frame(summary(fit_seir_V_2_peaks, pars = "pred_cases", probs = c(0.025, 0.05, 0.1, 0.5, 0.9, 0.95, 0.975))$summary), t=1:(n_days-1))
colnames(SMR_pred_2) <- make.names(colnames(SMR_pred_2)) 
```
...
ggplot(SMR_pred_2, mapping = aes(x = t)) +
...
```{r}
#summary for p_reported
summary(fit_seir_V_2_peaks, pars = c("p_reported"))$summary
```


```{r}
#posterior density for p_reported with mean, median and 95% CI
stan_dens(fit_seir_V_2_peaks, pars = 'p_reported', alpha = 0.25) +
  geom_vline(aes(xintercept= summary(fit_seir_V_2_peaks, pars = c("p_reported"))$summary[6],
                 color="median"), linetype="dashed",
             size=1)+ 
  geom_vline(aes(xintercept= summary(fit_seir_V_2_peaks, pars = c("p_reported"))$summary[1],
                 color="mean"), linetype="dashed",
             size=1)+
  geom_vline(aes(xintercept= summary(fit_seir_V_2_peaks, pars = c("p_reported"))$summary[4],
                 color="95% CI"), linetype="dashed",
             size=1)+
  geom_vline(aes(xintercept= summary(fit_seir_V_2_peaks, pars = c("p_reported"))$summary[8],
                 color="95% CI"), linetype="dashed",
             size=1)+
  labs(x = "p", y = "Density")+
...
```{r}
c(mean(rstan::extract(fit_seir_V_2_peaks)$R0_1), mean(rstan::extract(fit_seir_V_2_peaks)$R0_2))

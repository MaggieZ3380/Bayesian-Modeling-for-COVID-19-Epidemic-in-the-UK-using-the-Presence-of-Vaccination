...
int real_to_int(real x, int min_val, int max_val){
  ...
  real[] sir(real t, real[] y, real[] theta, 
      ...
      real init[5] = {(0.7*N), (N*0.3)-I0-E0, E0, I0, 0};
      real P = y[1] + init[1];
      real S = y[2] + init[2];
      real E = y[3] + init[3];
      real I = y[4] + init[4];
      ...
      if (tindex < 58 + delta){
        real dP_dt = -lambdaP*S*I/Ne - 0.7*alpha*x_r[tindex+1];
        real dS_dt = lambdaP*S*I/Ne - lambda1*S*I/Ne - 0.3*alpha*x_r[tindex+1];
        real dE_dt = lambda1*S*I/Ne - sigma*E ;
        ...
        real dP_dt = -lambda2*S*I/Ne - 0.7*alpha*x_r[tindex+1];
        real dS_dt = lambda2*S*I/Ne - lambda1*S*I/Ne - 0.3*alpha*x_r[tindex+1];
        real dE_dt = lambda1*S*I/Ne - sigma*E ;
        real dI_dt = sigma*E - mu*I;
        real dR_dt = mu*I + alpha*x_r[tindex+1];
...
transformed parameters{
  ...
  for (i in 1:n_days-1){
    incidence[i] =  (y[i,1] - y[i+1,1] + y[i,2] - y[i+1, 2] + y[i,3] - y[i+1,3])* p_reported; 
...
model {
  ...
  lambda2 ~ normal(2,0.5);
  //lambda3 ~ normal(0.15,0.5);
  //lambdaP ~ normal(0, 0);
